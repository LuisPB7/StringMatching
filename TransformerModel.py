...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.embed = nn.Linear(CHAR_SIZE, 512)
        self.positional_encodings = PositionalEncoding(512)
        self.relu = nn.ReLU()
    
    def forward(self, s, lengths):
        s = self.embed(s)
        s = self.relu(s)
        s = self.positional_encodings(s)
        s = self.transformer_encoder(s)
        s = self.relu(s)
        sentence = torch.zeros_like(s).cuda()
        for i, len in enumerate(lengths):
            sentence[i][:len] = s[i][:len]
        sentence = nn.MaxPool2d((sentence.size()[1],1))(sentence).squeeze(dim=1)
        return sentence

class TransformerModel(pl.LightningModule):
    def __init__(self, name, train_dataset, test_dataset):
        super(TransformerModel, self).__init__()
        self.lin1 = nn.Linear(512*4, 512)
