...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, batch_first=False, **kwargs):
        ...
        if self.vq: vs = self.vq(vs)
        elif self.vq_collapsed: vs = self.vs
        if self.q:
            query = self.q(query)
        ...
        if self.drop: q, k, v = self.drop(q), k, self.drop(v)
        original_q = q
        if not batch_first: q, k, v = q.transpose(0, 1), k.transpose(0, 1), v.transpose(0, 1)
...
class Block(nn.Module):
    def __init__(self, embed_dim, hidden_dim, heads=1, dropout=None, rnn=False, residual=True, use_attn=True):
        super().__init__()
        self.attn = None
        if use_attn: self.attn = Attention(embed_dim, heads=heads, r=False, dropout=dropout)
        ...
        self.rnn = None
        if rnn:
            self.rnn = nn.LSTM(input_size=embed_dim, hidden_size=int(hidden_dim / 2), batch_first=False, bidirectional=True)
            if rnn not in [True, False]: self.rnn = rnn
...
class SHARNN(nn.Module):
    def __init__(self, ninp, nhid, nlayers, dropout=0.5, dropouth=0.5, dropouti=0.5, dropoute=0.1, wdrop=0, tie_weights=False):
        ...
        self.encoder = nn.LSTM(input_size=embed_dim, hidden_size=int(hidden_dim / 2), batch_first=False, bidirectional=True)
        for idx in range(num_layers):
            rnn = True
            self.blocks.append(Block(hidden_dim, hidden_dim, self.num_heads, dropout=dropouth, rnn=rnn, residual=False, use_attn=True if idx == num_layers - 2 else False))
